...
class MSELossNode(MetaNode):
    ...
    Takes a predicted value (y_hat) and a target value (y),
    computes the squared error scaled by 1/2, and propagates gradients
    ...
    def __init__(self, y_hat: ValueNode, y: ValueNode, out: ValueNode):
        ...
        Args:
            y_hat: predicited values node.
            y: Second multiplicand node.
            out: Loss between y_hat and y.
        """
        super().__init__()
    ...
    def backward(self, grad_z):
        """Apply loss function and route gradients to both parents."""
        y_hat_val, y_val = self.get_parent_values()
